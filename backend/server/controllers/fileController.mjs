...
export const uploadHealthData = (req, res) => {
	...
	ehrUpload(req, res, (err) => {
		...
		console.log("Uploaded health data file at: ", join(ehrDir, req.file.originalname));
		res.status(200).json({ message: "Health data file uploaded successfully" });
		(async () => {
			try {
				const data = await fs.promises.readFile(path.join(ehrDir, req.file.originalname), "utf-8");
				await summarizeFHIR(req._dbUser, data);
			} catch (error) {
				console.error("Error getting llm analysis data:", error, req._dbUser, req.file.originalname);
			}
		})();
	});
};
...
export const getHealthFileByName = (req, res) => {
	...
	const filePath = path.join(__dirname, "../../uploads", req._dbUser.userid.toString(), "ehr", filename);
