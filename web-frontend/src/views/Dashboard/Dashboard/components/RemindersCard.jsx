...
import { Box, Flex, Text, useColorModeValue, VStack, Heading, Icon } from "@chakra-ui/react";
    ...
    <Box
      ...
      >
        <Flex alignItems="center">
          ...
          <Heading size="md" color="white">Medication Reminders</Heading>
        </Flex>
      </Box>
      ...
      {reminders.length > 0 ? (
        reminders.map((reminder, index) => (
          <Box
            key={index}
            bg={index % 2 === 0 ? bgColor : useColorModeValue("gray.50", "gray.600")}
            p={4}
            width="100%"
          >
            <TimelineRow
        ...
        <Text color={textColor} textAlign="center" p={4}>No reminders available.</Text>
      )}
      </VStack>
    </Box>
  );
